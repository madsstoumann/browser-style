...
- [Vercel: HTTP Security Headers](../guides/vercel.md#http-security-headers-csp-permissions-policy-referrer-policy) — Edge Config batch read in middleware, per-request nonce
- [Cloudflare: HTTP Security Headers](../guides/cloudflare.md#http-security-headers-csp-permissions-policy-referrer-policy) — KV read in Worker fetch handler, `crypto.randomUUID()` nonce
