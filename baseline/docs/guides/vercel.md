...
For architecture context, see [Webhook & Edge Delivery](../guides/webhook.edge.md).
...
Custom error pages are resolved from the `error_pages` map on the site config. See [Error Page](../content/error-page.md) for the full model and all status code examples.
