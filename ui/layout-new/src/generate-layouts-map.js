...
function extractLayoutData(layoutFile) {
	...
	for (const layout of layoutFile.layouts) {
		...
		layouts[key] = {
			...
			items: layout.items,
			repeatable: layout.repeatable || false
		}
	}
...
function generateLayoutsMap() {
	...
	for (const file of files) {
		...
		if (layoutFile.layouts && Array.isArray(layoutFile.layouts)) {
			...
			Object.assign(layoutsMap, extracted)
 ...
 * Value: { srcset, items, repeatable }
 */
export const layoutsMap = ${JSON.stringify(layoutsMap, null, 2)}
...
/**
 * Get srcset data for a layout pattern
...
export function getLayoutSrcset(pattern) {
	return layoutsMap[pattern]?.srcset || null
}

/**
 * Get all data for a layout pattern
 * @param {string} pattern - Layout pattern (e.g., "grid(3c)")
 * @returns {Object|null} Layout data or null
 */
export function getLayoutData(pattern) {
	return layoutsMap[pattern] || null
}
`
	...
	console.log(`   Total layouts: ${Object.keys(layoutsMap).length}`)
