...
Scroll-driven item animations via `animation-items` attribute. Creates a named `view-timeline: --layout-tl` on the container; children animate using that timeline. All keyframes are self-contained (opacity baked in).
