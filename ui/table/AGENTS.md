...
- **Registered web component** (JS imported): `customElements.define('ui-table', UiTable)` upgrades the element, but the instance is inert unless `mount` or `frame` is present — a bare `<ui-table>` does zero work, not even attribute forwarding. With `mount`, the component forwards `variant`, `hover`, `size`, `tint`, `tint-end`, `tint-tr`, `tint-bl`, `c1`–`c8` to the child `<table>` as `data-*`; `variant` is also mirrored to the wrapper (since `data-variant~="rounded"` applies to both). `sticky` stays a plain attribute on `<ui-table>` — CSS matches it directly via `[sticky~="cN"]`. `tint-axis` is *routed* (not forwarded): values `vertical`/`horizontal`/`2d` place the `tinted` attribute on the first `<tbody>` (vertical/2d) or first `<colgroup>` (horizontal) — the component owns that placement, clearing the old target before setting the new. With `frame` (which implies `mount`), a `ResizeObserver` additionally auto-computes sticky column offsets and toggles an `[overflowing]` attribute as a Safari ≤ 18 fallback for browsers without `animation-timeline` support. Attribute lists live in `FORWARD_TO_TABLE` / `FORWARD_TO_SELF` / `CHILD_ROUTING` at the top of `index.js`. The component also respects any `data-*` already set on the inner `<table>` at mount time — author's direct markup wins over forwarding on first render.

## Attribute surface

| Attribute | Where | Purpose |
|-----------|-------|---------|
| `data-variant` | table | Space-separated layout tokens (`rounded`, `split-cols`, `split-rows`, `th-dark`, `th-light`, thead-divider family (width: `th-divide-lg/xl`; style: `th-dotted/dashed/double/groove/ridge`), cell-border style: `td-dotted`/`td-dashed`, `caption-bottom`, `fixed`, `block-border`, `no-border`, `no-wrap`, zebra variants) |
