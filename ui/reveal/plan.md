...
> (implemented as `[variant~="flp"]`-style selectors in `ui-reveal.css`).
...
>
> The `type="popup"` sketched below shipped as `exp` + `pop`, and the
...
Idea: morph the popup card (now `variant="exp pop"`) from its grid cell straight to a full-bleed
...
> `ui-reveal[variant~="scr"][variant~="flp"]` (`ui-reveal.css`) — now paint
