...
- **One** element after `</summary>` — the panel. Use `<ui-content>` to give the back the card typography + `data-part` parts; the front-only `ovr()` overlay is reset on the back, so it renders as a normal flow column on the panel background. The panel's own `content=` placement still applies — `content="plc(cc) tal(ctr)"` centres the answer of the quiz flashcard. The close-icon clearance (`--ui-reveal-icon-clear`, indenting the panel's first child on the icon's side) becomes symmetric on a `tal(ctr)` panel, so centred lines centre on the plate, not on the shortened box.
