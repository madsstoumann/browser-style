...
A CSS-first **disclosure** built on native `<details>` / `<summary>`, composed over the same card engine as `<ui-card>`. The `<summary>` is the trigger face; the revealed panel is `::details-content` (everything after `</summary>`). Four reveal animations — `expand`, `flip`, `slide`, `scale` — plus a full-card trigger and an expand-to-popup mode. No JavaScript required.

`<ui-reveal>` `@import`s `ui-card.css`, so it shares the card primitives and DSLs verbatim: the **`media=`** frame ([media.md](../card/media.md)), the **`content=`** text column ([content.md](../card/content.md)), and the **`variant=`** arrangement / overlay / theme / corners ([ui-card-tokens.md](../card/ui-card-tokens.md)). This package adds only the reveal-specific parts: the `<details>` / `<summary>` wiring, `<ui-icon>`, `<ui-face>`, and the reveal animations.
...
```html
<ui-reveal type="expand" icon="top right">
  <details>
    <summary>
      <ui-content>
...
For `flip` / `scale` / `slide`, wrap the front face in `<ui-face>` so it can transform independently of the toggle icon. Overlay **markers** (`<ui-chip>`, `<ui-sticker>`) are valid inside the `<summary>` media; overlay **controls** (`<ui-save>`, `<ui-play>`) are **not** — they are interactive, which is invalid in `<summary>` (a click there toggles the `<details>`).

```html
<ui-reveal type="flip" icon="top right sm" icon-close="dark"
           variant="ovr(tl) rds(lg-sq)" media="asr(1/1) hov(zoom) obp(tc)" scroll>
  <details name="flip">
    <summary>
      <ui-face>
        <ui-media>
...
- `<summary>` — the front face. For `flip` / `scale` / `slide`, wrap it in `<ui-face>`. `<ui-media>` / `<ui-content>` inside come from the card engine.
...
|---|---|---|
| `type` | *(none)* `expand` `flip` `slide` `scale` | Reveal animation (see below). |
| `type-lg` | `scale` | Swaps the animation to `scale` at the `lg` width (≥ 44rem container), overriding the base `type`. |
| `from` | `left` `right` *(default)* `top` `bottom` | Direction for `flip` / `slide`. |
| `trigger` | `card` | Whole card toggles, front and back — no `<ui-icon>` needed. |
| `to` | *(present)* | `type="expand"` only — opens the card as a fixed, centered popup with a backdrop and pop-in. |
| `scroll` | *(present)* | Locks a long panel to the card frame and scrolls the overflow. `type="flip"` (any width) and `type-lg="scale"` (≥ 44rem). |
| `icon` | position + style + size tokens | Positions and styles the toggle icon (see below). |
| `icon-close` | same tokens as `icon` | Same tokens, applied only while the card is **open** (re-place / re-colour the icon on the back). |

> `name` is the native `<details>` attribute (set on the inner `<details>`), not a reveal attribute. With `type="expand" to`, the in-flow `<ui-reveal>` stays as a placeholder (reserves the cell via `aspect-ratio`) and only the inner `<details>` goes `position: fixed`, so the surrounding grid never reflows.

### `type` — animation

| Type | Behaviour |
|---|---|
| *(none)* | Plain disclosure — content shows/hides, no special motion. |
| `expand` | Panel expands open below the front face (height animation to `auto`). With `to` it morphs into a fixed popup. |
| `flip` | Card flips 180° to reveal the back. Wrap the front in `<ui-face>`. Direction via `from`. |
| `slide` | Panel slides in over the face from an edge. Direction via `from`. |
| `scale` | Panel morphs out from the icon's corner, scaling to fill the card. Morph origin follows the `icon` position. |

### `icon` / `icon-close` tokens

Space-separated tokens on the `icon` attribute. `icon-close` takes the same tokens but applies only while open.
...
```html
<ui-reveal type="flip" variant="ovr(bl) rds(md-sq) thm(dark)"
...
| `interpolate-size: allow-keywords` | `expand` height animation to `auto` | Chrome 129+ |
| `:has()` | `to` popup state, `trigger="card"` | Chrome 105+, Safari 15.4+, Firefox 121+ |
| Container queries | responsive front face, `type-lg` | Chrome 105+, Firefox 110+, Safari 16+ |
| `corner-shape: superellipse(…)` | `rds(*-sq)` squircle corners | Chrome 135+ (falls back to normal rounding) |
| `animation-timeline: scroll()` | `scroll` fade-shadow mask | Chrome 115+ (degrades to a plain scroll) |
