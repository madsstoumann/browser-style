...
- `variant="hide-summary"` hides the open item's summary via `details[open] > summary { display: none }` (direct child, so nested accordions keep their summaries). It is a pure visual variant with no JS coupling — but it is only usable paired with `no-collapse` + `name`: hiding the summary removes the only toggle, so without an exclusive non-closable group the open item becomes a dead end. This pairing is documented, not enforced. Composes with any layout, including `type="split"`.
