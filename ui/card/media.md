...
| `<ui-beacon>` | animated dot / pill / solid label (reuses `ui/beacon`) | the chip's **live** counterpart. Card tokens: hue `beacon(red…white)` (same `--ui-theme-*` bundles as chip/sticker), variant `beacon(pll)` pill / `beacon(sld)` solid, animation `beacon(bln)` blink / `beacon(pls)` pulse / `beacon(brt)` breathe / `beacon(non)` off (solid defaults to blink), size `beacon(xs\|sm\|md\|lg)`. Over imagery prefer `pll`/`sld` — the bare dot has no contrast plate. **Marker-class as furniture**: the renderer emits text-only markup and the WC skips the click-to-pause checkbox inside `ui-media`/`summary`; pausing still works via `prefers-reduced-motion` and `[paused]`. |
