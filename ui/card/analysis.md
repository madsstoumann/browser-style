...
| `animate` | `animations.css` | Scroll/keyframe animation applied to an element's children; keyframe-name tokens + multiplier modifiers (`(2)`/`(3)`, `bounce(...)`). | Token-list |
| `animate-self` | `animations.css` | Same animation API targeting the element itself; also a companion flag stagger checks to yield container-entry ownership. | Token-list |
...
- **Shared vocabulary across systems.** `stagger`/`data-stagger`, `animate`, `animate-self`, `nav`, `arrow`, `dot`, `overflow` and `theme` originate in **base** and are re-used by both **layout** and **card**. The layout carousel controls (`nav`/`arrow`/`dot`), the `stagger` engine and the animation `@keyframes` all live in `ui/base` (`carousel.css`, `stagger.css`, `animations.css`). As of v4 these are **no longer bundled into `layout.css`** — `@browser.style/base` must be loaded alongside `layout.css` (it always is in practice); `layout/core/animations.css` holds only the animation *wiring* and references base's keyframes by name.
