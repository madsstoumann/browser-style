...
| `subgrid` | `<lay-out>` | Row count each child adopts as shared subgrid rows (activated per-breakpoint by the bare `subgrid` token — one-way, no off token). | Value |
...
| `data-clone` | `ui/carousel` (`carousel.css`) | Internal marker on cloned carousel slides (suppresses their scroll-marker). | Boolean (internal) |
