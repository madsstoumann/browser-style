...
| `readingTime` | string | |
| `media` | array | `{asset\|src, mediaType: image\|video\|audio\|youtube\|vimeo, alt, caption}` — more than one item ⇒ carousel. Video items also take `{autoplay, muted, loop, controls, poster}`; an `audio` item is an `<audio controls>` hidden while scripting runs (poster image stays the visual; `furniture.play` drives it; with scripting disabled the native bar surfaces at the frame's block-end — scoped `associatedMedia` → `AudioObject`); youtube/vimeo items set `provider`/`video` lite-embed attributes on the frame |
...
*derived from* micro rather than maintained beside it, and cannot drift; microdata added
later is stripped with no extra work.

The strip is safe by construction: `esc()` turns every `"` in content into `&quot;`, so a
bare `"` can only come from a renderer-authored attribute — the patterns cannot match text.
It removes **attributes**, never elements, because `floorSize()` returns an unclosed
`<span>` that its caller closes; deleting annotated elements would unbalance the string.
Wrappers that existed only to hold microdata are then swept once they are empty, and
`machineOnly()` — the crawler-only address/hours group — returns nothing outside micro mode.

Guaranteed across the whole 144-card corpus by `render.test.js` § *schema modes*: raw output
carries no `itemprop`/`itemscope`/`itemtype`, leaves no empty or hidden residue, keeps the
markup balanced, and **loses no visible text**.

The mode is module-level state, set from `options` like `images` and `typeChip`. The three
exports that bypass `renderCard` — `reviewItems`, `realestateSections`,
...
| faq, recipe, job, howto | [`ui/accordion`](../../accordion/) | `<ui-accordion group variant?><cq-box><details>…` — the `cq-box` is hand-authored by the renderer so the CSS-only form styles without the accordion JS; variant from `parts.accordion`. With the `popover` word each item becomes `<button popovertarget>` + `<div popover>` (label + close button in the panel header) instead of `<details>` — ui-accordion.css § Popover mode |
