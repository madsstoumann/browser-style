...
Shape: `{ element, variant, media, content, text?, parts?{quote, accordion, buttonGroup, buttonGroupSize, buttonGroupTheme, office, officeTheme}, styles?, reveal?{type, typeLg, icon, iconClose, scroll…} }` — `parts` carries sub-component variant words written verbatim to the emitted `<ui-quote>`/`<ui-accordion>` `variant=` attribute (and the variant picker's `.ui-button-group` `data-variant=`), validated by `tokens.lint.js` against the component vocabularies (`parts.accordion` also accepts the mode word `popover` — stripped from `variant=`, it swaps the accordion's `<details>` items for `<button popovertarget>` + `<div popover>` pairs, native Popover API, no JS) — the structured `reveal{}` object stays, but `render.js` folds it into `variant=` tokens (`exp`/`flp()`/`sld()`/`grw()` — type+from fold into ONE token — plus the `lg:grw` swap, `pop`, `trg(card)`, `scr`, `ico()`, `icc()`) at render time; the preset schema has no `nav`/`arrow`/`dot` fields (carousel controls are `media=` tokens). Collections: `data/card.presets.json` (canonical) and `data/card.presets.demo.json`. Content instances reference one via `"preset": { "$ref": "card-preset/hero" }` — swap the ref to restyle without touching content. Schemas live in `cms/baseline/models/` (`card.schema.json`, `card-preset.schema.json`).

`render.js` is a **string-producing SSR engine** — no `document`, runs unchanged in Node. `renderCard(ucf, presets, cards)` resolves the preset, dispatches on `preset.element` (`ui-card` | `ui-reveal` | `ui-media` | `ui-content`), and appends furniture tokens from the content (e.g. a `chip` field → `chip(ts) chip(green)` on `media=`). The id-keyed `cards` map resolves BOTH card references: the `flipside` envelope ref and `{ "$ref": "card/<id>" }` rows in ref-enabled `details` arrays (expanded before the `DETAILS` renderers via the generated `data/details.refs.data.js` projection table — docs/card.model.md § Referenced rows; `expandDetails` is exported for build scripts). Everything passes through `esc()`. Demo data: `data/demo/*.json`, manifest `data/index.json`, driver `demo/render.html`. Full walkthrough: `docs/card.md`.
...
| `render.js` | Node-safe SSR: JSON (UCF) → HTML string. Type-specific markup hangs off two per-type maps: `DETAILS` (the text column) and `REVEAL_FACES` (a `<ui-reveal>`'s two faces, for types whose split the generic front/back cannot express — docs/schema.md § Quiz). Published (in `package.json` `files`/`exports`) together with `data/`. Optional 4th arg `{ images }` arms the Cloudflare srcset pipeline (off = byte-identical legacy output) |
...
| `details.build.js` | generates everything derived from `data/details.json` (the per-type `details` manifest): the card editor's schemas + lookups (`cms/editors/card/src/details.data.js`), the `schemaType` options and `details` digest inside `cms/baseline/models/card.schema.json` (targeted splice), and the `<!-- details:… -->` tables in `docs/card.model.md`. Idempotent — run twice, second run a no-op |
| `details.lint.js` | **details sync gate** — manifest ⇔ `SCHEMA_TYPES`/`SUBTYPES`/`SUBHEADLINE_SLOT`/`PAYWALL_TYPES`/`DETAILS`, every lookup resolves, every `details` key in every `data/*.json` instance declared with a matching kind, `*Display` discipline, `card.schema.json` options, and the committed generated file. Advisory: renderer `d.*` reads and `schema.html` section grouping |

## Conventions & pitfalls

1. **Hand-author `<cq-box>`** inside `<ui-card>` — nothing auto-inserts it.
2. **Never `innerHTML` with data** — `render.js` escapes everything via `esc()`; only `<b>` in headlines survives. Keep it that way. The two **display formatters** `num()` and `fmtPrice()` return **already-escaped** strings (both fall through to raw author data on unformattable input, and all ~16 call sites interpolate them into text nodes) — interpolate them bare and never `esc()` them again, or you double-escape. When composing them into a joined string, escape the *other* components individually rather than wrapping the join. A priced row goes through `priceValue()`, which pairs the machine number on a `<meta>` with `fmtPrice()`'s human string as the text node — a formatted price must never sit in a `<data itemprop>` ([schema.md § Price](docs/schema.md#price)).
...
7. **One position grid, no exceptions.** `ovr()`, furniture, `scm()`, `mrk()`, `arw()`, `plc()`, `obp()` and reveal's `ico()`/`icc()`/`grw()` all use the logical `ts tc te · cs cc ce · bs bc be` set; families use *subsets* of it (4 corners for `ico()`, 6 cells for sticky `play()`), never their own words. The physical `tl…br` aliases were **removed in v5** — from `ovr()` first, then from `obp()`. Where a property has no logical form (`object-position`, `linear-gradient()` directions) the cell resolves through base's `--_dir-s`/`--_dir-e` pair rather than a per-family `:dir(rtl)` arm. For an image-space focal point that must not mirror, set `--ui-media-op` directly. Still on invented words and not yet migrated: `marquee(top|bot)`, `flp(top|btm|lft|rgt)`, `sld(top|btm|lft|rgt)`.
...
9. **`<ui-play>` has one contract:** `command="--play-pause" commandfor="<video/audio id>"`, handled by `video.js`. The `--` prefix is what keeps the markup valid — the proposed bare `play-pause` is not yet a conforming `command` value, and the custom-command namespace is the spec's own escape hatch; video.js accepts both spellings and defers to native handling only for the bare one. No `ui-play-toggle` event. The carousel's control is the one target-less exception, auto-discovered by `carousel.js`.
