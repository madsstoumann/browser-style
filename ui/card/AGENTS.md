...
| `lightbox.js` | popover-lightbox niceties, all gated on `ui-media[popover]` (moved from `ui/lightbox/command.js`): DOM carousel controls via `/polyfill/carousel-controls.js`, `media-open=` swap, `--lightbox-layout`, View Transition morph, modality, back-button close, tile jump, pause-on-close |
