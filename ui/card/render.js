...
import DETAIL_REFS from './data/details.refs.data.js';
...
/* Resolve a card → card reference ({ "$ref": "card/{id}" }) against a UCF map keyed by id.
   Canonical form is "card/<id>"; a bare id is tolerated, same posture as resolvePreset. */
const resolveCard = (ref, cards) => {
	const raw = ref?.$ref || '';
	const id = raw.startsWith('card/') ? raw.slice('card/'.length) : raw;
	return (id && cards[id]) || null;
};

/* ── referenced detail rows ──
   A details array field declared with `ref` in data/details.json accepts
   { "$ref": "card/<id>" } rows beside inline rows. The row is materialized by
   PROJECTING the referenced card's fields through the generated table
   (data/details.refs.data.js); keys stated inline on the ref row win. One level
   deep like flipside — projection reads the target's raw fields, never recurses.
   Docs: docs/card.model.md § Referenced rows */

const atPath = (obj, path) => path.split('.').reduce((o, k) => (o == null ? o : o[k]), obj);

/* copy-on-write replacement of one nested path — the surrounding objects are cloned,
   everything else keeps its identity */
const setAtPath = (obj, path, value) => {
	const keys = path.split('.');
	const clone = { ...obj };
	let src = obj, dst = clone;
...
/* one $ref row → projected inline row; null = unresolvable */
const projectRow = (row, spec, cards) => {
	const target = resolveCard(row, cards);
	if (!target) return null;
	const source = target.fields ?? target ?? {};
	const out = {};
	for (const [key, path] of Object.entries(spec.project || {})) {
		let value = atPath(source, path);
		if (value?.$richtext) value = plain(value);
		if (value !== undefined) out[key] = value;
	}
	for (const [key, value] of Object.entries(row)) if (key !== '$ref') out[key] = value;
	return out;
};

/* expand every declared ref array for one card; identity when no row carries $ref,
   so untouched instances render through unchanged data. Unresolvable ref → row
   dropped with a loud HTML comment (the lint catches it statically). */
const expandRefs = (fields, type, cards) => {
	const specs = DETAIL_REFS[type];
	const d = fields.details;
	if (!specs || !d) return { fields, note: '' };
	let details = d, note = '';
	for (const [arrayPath, spec] of Object.entries(specs)) {
		const rows = atPath(details, arrayPath);
		if (!Array.isArray(rows) || !rows.some((row) => row && typeof row === 'object' && '$ref' in row)) continue;
		const expanded = [];
		for (const row of rows) {
			if (!(row && typeof row === 'object' && '$ref' in row)) { expanded.push(row); continue; }
			const projected = projectRow(row, spec, cards);
			if (projected) expanded.push(projected);
			else note += `<!-- unresolved card ref: ${esc(String(row.$ref))} -->`;
		}
		details = setAtPath(details, arrayPath, expanded);
	}
	return { fields: details === d ? fields : { ...fields, details }, note };
};

/**
 * Expand { "$ref": "card/<id>" } detail rows against a UCF map — what renderCard does
 * internally, exported for build scripts and consumers that read `details` directly.
 * @param {object} fields — a UCF `fields` object (or a whole UCF document's fields)
 * @param {object} [cards] — id → UCF map
 * @returns {object} fields with declared ref rows projected inline (identity when none)
 */
export const expandDetails = (fields, cards = {}) => expandRefs(fields, baseType(fields), cards).fields;
...
function renderCardHtml(ucf, presets = {}, cards = {}) {
	let fields = ucf?.fields ?? ucf ?? {};
	const cardId = ucf?.id || null;
	CARD_ID = cardId;
	const type = baseType(fields);
	/* referenced detail rows materialize BEFORE anything reads details */
	const expansion = expandRefs(fields, type, cards);
