...
| `ui/button-group` | `.ui-button-group` — the product-page size picker (`details.variants.control: "buttons"`); `data-variant=`/`data-theme=` because the host is a `<fieldset>` | `button-group/ui-button-group.css` |
