		...
		"content": {
			...
			"tokens": {
				...
				"tal": {
					...
					"notes": "Explicit text-align: start (the no-token default), ctr, end. Outranks ovr()'s implied per-cell text alignment (unlike plc, which sits under the ov-* slots) and the legacy ctr/end flags' text half."
				},
				"scr": {
					"axis": "scroll",
					"element": null,
				...
				"ctr": {
					...
					"canonical": null,
					"sources": [
						"ui/card/content.css:97-98"
					],
					"notes": "Legacy bare flag: sets --ui-content-align: center — BOTH the column's cross-axis (align-items) and text-align in one prop. plc()/tal() are the fine-grained forms and win when present. Kept live (not deprecated); prefer plc()/tal() in new code."
				},
				"end": {
					"axis": "alignment",
					"element": null,
					...
					"notes": "Legacy bare flag: sets --ui-content-align: end — BOTH the column's cross-axis (align-items) and text-align in one prop. plc()/tal() are the fine-grained forms and win when present. Kept live (not deprecated); prefer plc()/tal() in new code."
				}
			},
			"bareFlags": {
				"ctr": {
					...
					"canonical": null,
					"sources": [
						"ui/card/content.css:95",
						"ui/card/content.css:131",
						"ui/card/content.css:153"
					],
					"notes": "Centres the whole content column: --ui-content-align feeds BOTH `align-items` (cross axis) and `text-align` on <ui-content>, each behind the ovr() overlay props (`var(--ui-content-ov-align, var(--ui-content-align, normal))`), so an overlay placement WINS over ctr/end — they are for the standalone / non-overlaid case. No md:/lg: forms."
				},
				"end": {
					"axis": "alignment",
					"element": null,
					...
					"canonical": null,
					"sources": [
						"ui/card/content.css:96",
						"ui/card/content.css:131",
						"ui/card/content.css:153"
					],
					"notes": "End-aligns the whole content column (same mechanism and same ovr() precedence as ctr). Beware the bare word: `end` is a very short whole-token needle shared with other vocabularies in other attributes — a manifest consumer must key it per attribute."
				}
			}
		}
	}
