		...
		"media": {
			...
			"tokens": {
				...
				"lightbox": {
					...
					"requiresJs": {
						"carousel controls in the open lightbox": "ui/lightbox/command.js (injects /polyfill/carousel-controls.js DOM controls; without it the open carousel is swipe/keyboard/scrollbar)",
						"--lightbox-layout / [open] reflection / View Transition morph": "ui/lightbox/command.js"
					},
					"deprecated": false,
					"canonical": null,
					"sources": [
					...
					"notes": "Default area bs; INTERACTIVE, so invalid inside a <summary> (ui-reveal front face). \"View gallery\" invoker for the popover lightbox: <button command=\"toggle-popover\" commandfor=\"<frame id>\"> on a <ui-media popover> frame — the open-state presentation is the open: token family (media.lightbox.css). Glyph = inline <ui-icon><svg viewBox>…</svg> from /assets/svg: library-photo (\"open gallery\", renderer default, furniture.lightbox.shape=photos) or window-maximize (\"full screen\", shape=maximize); while open the svg hides and ui-lightbox.css draws ui-icon's cross bars (close affordance). In a nav scroller the element is sticky-pinned to the scrollport and must sit BEFORE the slides (first child; six edge cells; end corners are not relocated — prefer start corners, media.carousel.css). Native scroll-control pseudos do NOT follow a popover frame into the top layer (current Chromium) — ui/lightbox/command.js injects the /polyfill/carousel-controls.js DOM controls on popover frames in every browser, and media.lightbox.css suppresses the native pseudos on those [data-ui-carousel-polyfill] frames. Open-state nav switching uses the companion media-open= ATTRIBUTE (ordinary control spellings, swapped by command.js — never an open: token, control stems are substring-matched). Single-ink control: no pale/muted arms, like save/play. No lightbox(pll)."
				},
				"marquee": {
					"axis": "band",
					"element": "ui-marquee",
				...
				"arw": {
					...
					"canonical": null,
					"sources": [
						"ui/base/carousel.css:93",
						"ui/base/carousel.css:353",
						"ui/base/carousel.css:358",
		...
		"variant": {
			...
			"bareFlags": {
				...
				"row-r": {
					...
					"notes": "As `row`, plus `ui-content { order: -1 }` so the content column comes first. variant= is host-only by design — it arranges the two children, so there is no self arm on <ui-media>/<ui-content>."
				},
				"exp": {
					"axis": "reveal-animation",
					"element": null,
