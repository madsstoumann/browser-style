			...
			"tokens": {
				...
				"arw": {
					...
					"sources": [
						...
						"ui/carousel/carousel.css:685",
						"ui/card/media.carousel.css:93",
						"ui/card/media.carousel.css:151"
					],
				...
				"mrk": {
					...
					"sources": [
						...
						"ui/carousel/polyfill/carousel.css:199",
						"ui/carousel/carousel.css:342"
					],
		...
		"content": {
			...
			"tokens": {
				...
				"gate": {
					...
					"canonical": null,
					"sources": [
						"ui/card/content.css:234-238",
						"ui/base/scroll.css"
					],
					"notes": "The paywalled-preview look: the text column fades out towards its block-end. Reuses the shared scroll-fade GRADIENT (--ui-scroll-fade-mask, ui/base/scroll.css) but holds it STATIC — writes only --ui-scroll-fade-end (a registered non-inheriting <length>, normally animated on a scroll() timeline) and applies the mask; no max-block-size, no overflow, no animation, so no @supports gate and no reduced-motion arm. Deliberately NOT scr: a gate that scrolls is not a gate. Default size 100cqb — <ui-card> is an inline-size container only, so cqb has no block-axis container and resolves to the small-viewport block size, which drives calc(100% − …) negative and clamps to one opaque→transparent ramp over the whole column; any <length> ≥ the column does the same, and 100% is not accepted by the registered syntax. Override per element with --ui-content-gate-size (unregistered, inherits). Whole-token (~=) bare flag; two arms because mask is a real property on the <ui-content> box. mask-clip: no-clip is load-bearing: the default border-box clip also clips HIT-TESTING of anything a descendant paints outside the column — the `cover` link's ::after overlay that makes the whole card clickable — so with the default clip the media stopped being a link (measured, Chromium). Nothing visible lives outside the column, so no-clip changes no pixels. Pair it with details.paywalled (docs/schema.md § Paywall) — the flag is the machine claim, this token the visual; the renderer never appends one from the other. Presentation only: a CSS mask hides nothing from a crawler or view-source — enforcement is server-side."
				}
			},
			"bareFlags": {}
		},
