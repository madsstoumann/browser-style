			...
			"tokens": {
				...
				"arw": {
					...
					"sources": [
						...
						"ui/carousel/carousel.css:677",
						"ui/card/media.carousel.css:93",
						"ui/card/media.carousel.css:150"
					],
					"notes": "Only STYLES arrows — presence comes from nav (bare|arw|blw|abv). No arw(md) (2.25rem is the host default) and no arw(ce): arw(set)'s inline side is start via ts/cs/bs, centre via tc/cc/bc, end by default. arw(ts…be) only carries the BLOCK row for split arrows; under axis(y) the inline letter is what matters (carousel.css:465). arw(blw)/arw(abv) reserve a band via media.carousel.css:93 (frame) and layout/core/base.css:312 (lay-out). arw(out) flanks the arrows OUTSIDE the frame on both inline sides, vertically centered, gap = --ui-carousel-outside-gap; the frame SHRINKS (inline-size + margin-inline:auto in media.carousel.css — padding-inline would sit on the scroll axis and let slides peek). ui-media frames only (no lay-out[overflow] arm yet) and horizontal only (:not(axis(y)) guard; axis(y) hides inline buttons anyway). Keeps the default disc chrome — compose arw(bare)/arw(lgt)/arw(drk) to restyle; undefined with nav(blw|abv)/arw(blw|abv)/arw(set). arw(rev) is @media (hover:hover)-gated."
				},
				"mrk": {
					"axis": "markers",
					"element": null,
