		...
		"map": {
			...
			"tokens": {
				...
				"tint": {
					...
					"notes": "Pure CSS, and it composes with ANY tiles() value at no extra request. vivid ADDS colour and blue RECOLOURS; every other value desaturates or shifts hue. blue works by sepia(1) first, because hue-rotate() rotates hue that is already there and a dark basemap is effectively greyscale — sepia forces a uniform cast the rotation can then move — one filter on .leaflet-tile-pane. NEVER on .leaflet-tile-container: leaflet.css:89 gives .leaflet-tile `filter: inherit`, so a filter there applies TWICE and invert(1) twice is the identity. The pane holds only tiles, so markers, zoom controls and the attribution keep their true colours. Unanimated, so the cost is a filter pass per tile paint, not per frame — but the filtered subtree does repaint per frame during a ZOOM tween. Docs: ui/map/readme.md § Tint"
				},
				"pin": {
					"axis": "pin",
					"element": null,
