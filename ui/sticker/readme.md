...
For a **named colour**, use `theme=` — the 9 theme colours `red orange green blue accent white gray slate black` (+ `pale`/`muted` modifiers), a background + paired ink in one keyword:
...
In a card, the same fill is available as the `sticker(fit)` media token (fits **all** lines):

```html
<ui-card media="asr(4/3) sticker(cc) sticker(fit)">
  <ui-media>
    <img …>
    <ui-sticker fill="#2F75BC" ink="#fff"><small>UP TO</small><strong>40%</strong></ui-sticker>
  </ui-media>
</ui-card>
```
