...
1. ~~**`industry` is emitted twice with conflicting values on a job card.**~~ **Done** — `envelopeProps()` derives the set of root-scope itemprops the envelope will claim from the same maps that drive its emission, and the colliding `DETAILS` renderers (`job`/`industry`, `social`/`author`) take it as a fifth argument and yield. Because the set is derived, re-adding `EYEBROW_PROP.job = 'industry'` cannot resurrect the bug. A corpus invariant in `render.test.js` renders every `data/*.json` and fails on any repeated property not declared multi-valued. Docs: `ui/card/docs/schema.md` § One property, one value.
...
2. ~~**Two reference strings lost inline markup.**~~ **Done** — `INLINE_TAGS` now re-allows attribute-free `<em>` and `<code>`, and the two `details` prose fields the reference page marks up (the flashcard `answer`, the glossary term `description`) render through `renderInline()` instead of `esc()`. The pattern matches *escaped* text and the bare forms permit no attribute, so there is no executable surface; `balancedInline()` additionally escapes the whole string when opens and closes do not tally, which also removes the orphan a rejected opening tag used to leave. Docs: `ui/card/docs/card.md` § Inline markup.
