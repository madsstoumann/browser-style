...
| R-14 | Dual-arm strategy: `:where()` factoring now; inherited-flag + `style()` queries in v5; canonical child placement in the renderer | — | M | v5 / presets |
...
- **`ui-marquee`** — **F-40 the odd one out on five axes** `[Debt]` `[M]` `[→ R-11]`: full-word token stem; bespoke `top/bot` position vocabulary instead of the 9-grid; excluded from the shared furniture rule (own rule at `z-index: 1`, `media.css:155-157`); absent from every furniture table in the docs, from `render.js`'s `buildFurniture`, and from every slide-exclusion list (F-20); and its `loop` arg collides with the carousel token (F-19). Decision needed: either promote it to a real furniture citizen (9-grid `ts/bs` rows spanning full width, 3-letter stem, exclusion lists, renderer support) or explicitly document it as a *band*, not furniture — a different layer with its own rules. Recommend the latter: it genuinely is a full-width band (like `nav(blw)`'s control band), and forcing it onto the 9-grid would be false uniformity. But its args must stop colliding either way.
...
Factor the queryable-descendant pair into one place: a shared selector alias used by every rule that today hand-writes `> cq-box` or `:is(cq-box, summary)` — the `align-content` divergence (F-14) is the existence proof that hand-maintenance fails. Cheapest robust form: stamp a common attribute on both wrappers at authoring time is *not* available (no JS), so the discipline is textual — a single `/* CQ-DESCENDANT */`-marked `:is(cq-box, summary)` pattern, plus a checklist in `ui-reveal.css`'s header enumerating the card rules that leak into the panel (the undo list, F-28), auditable on every new card token. Long-term (v5): move card rules to front-face-scoped subjects so the undo list shrinks toward zero.
...
Adopt in three steps (full analysis in §2):
