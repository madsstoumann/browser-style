...
> **ARCHIVE — read this for *why*, not for *what's next*.** Every F-xx finding and
> R-xx decision on the v4 card/layout line is recorded here with its rationale, and
> that is what it is good for. It is **not** a work list: the ledger below is
> essentially all `Done`, and later work has superseded parts of it (superseded
> entries carry an inline note — the surrounding text is left as written, because the
> reasoning is the point). What is still open now lives in
...
### 2d. Standalone primitives and the query container

**F-42 Responsive tokens silently fail on standalone primitives — and can match the wrong container** `[Contract-mismatch]` `[S]` `[→ R-14]`
Evidence: `ui/card/ui-card.css:179,216-224` (unnamed `@container` + `:is(cq-box, summary)` subjects), `ui/card/content.typography.css:218,276,501,509`, `ui/reveal/ui-reveal.css:123` (the named counter-example).
Two stacked failure modes for `md:`/`lg:` tokens on a standalone `<ui-media>`/`<ui-content>`: (1) every responsive rule's subject is `:is(cq-box, summary)`, which doesn't exist standalone — nothing can match; (2) even once self arms exist (R-14 step 3), the size queries are **unnamed**, so they resolve against the subject's *nearest ancestor* container — standalone with no container ancestor the query is false (silent no-op), and standalone inside an unrelated `container-type` ancestor (including someone else's card) the breakpoints fire off the **wrong box's** width. Reveal's animation dispatch is immune because it queries the *named* `bs-rvl` container. Separately, the `cqi` typography ramp does not fail standalone but changes meaning: container units with no eligible container fall back to the **small viewport size**, so the ramp tracks the viewport and typically sits at its clamp max on desktop — `content.md:365`'s "resolves at its preferred value" is incorrect and should be fixed alongside R-07. Fix: name the container (see R-14 step 5).

The support matrix this implies (worth stating in the docs): **`ui-card`/`ui-reveal` are the only query containers** — full `md:`/`lg:` + component-relative `cqi` support exists only inside them. **`lay-out-group` is *not* a container** (neither is `lay-out`, deliberately), so a group header's `content=` supports non-prefixed tokens only; its `cqi` ramp measures the viewport — practically fine for a full-bleed header, but fallback behavior, not container behavior. **Standalone primitives work fully minus CQ** (every non-prefixed token is an inherited custom prop), regaining responsiveness via the wrapper opt-in. Two designed pause-cases belong in the same doc note: `subgrid(on)` neutralizes the card's own `container-type` to keep the subgrid chain intact (`layout/core/base.css:345-351`), and reveal's open popup sets `container-type: normal` to release the fixed popup (`ui-reveal.css:249`) — in both states the `md:`/`lg:` tiers pause.
...
| R-14 | Steps 1–3 + 5 **done** (factoring shipped in a specificity-preserving form — the literal Option-1 shape would have dropped `hov(shape)`'s cascade win; self arms + `bs-card` naming + canonical renderer placement verified in-browser). Step 4 (style()-flag migration) **done, partial by design** — `hov()`, `tnt`/`hov(tint)`, `shp()`'s clip and `marquee()`'s placement migrated (**superseded:** `tnt`/`hov(tint)` were later reverted for WebKit — see the TODO entry); `asr()`, `rds(*-sq)`, `hov(tilt*)`'s perspective and the carousel bundle are structurally unmigratable (their real properties land on the attribute-bearing `<ui-media>`, and a container cannot restyle itself). See the TODO entry for the full account. |
| R-13 (manifest) | **Done (2026-07-27, second round)** — `ui/card/data/tokens.json` (54 stems / 13 bare flags / 247 arg values across the three attributes) + generated `tokens.data.js` mirror and `tokens.md` reference; `render.js`'s `FURNITURE_AXIS`/hue tables replaced with manifest lookups (byte-identical output proven across all 104 instances); `tokens.lint.js` (substring-shadowing + CSS-needle audit + sync checks) wired into `build.js` and `npm run lint:tokens`. The lint's first run caught the one remaining `[media*="loop"]` selector in `media.carousel.css`; the manifest audit also surfaced and fixed marquee's missing self arms (ancestor-only position args vs the renderer's canonical child placement) and the `pale`/`muted`/`sticker(fit)` merge-axis holes. The extraction produced a 32-item doc/CSS discrepancy list: items 1–16 are doc corrections for the next doc pass (now superseded-by-generation candidates — `tokens.md` is the counter-source), 17–32 are recorded implementation asymmetries, of which three need design calls: per-element axis maps for `pll`/`non` (one merge class, three element meanings), `bdr` on `<ui-reveal>` painting on the host box instead of `> details`, and group-header base size tokens not riding the responsive ladder (the dual declaration is scoped to card/reveal descendants only). |
| R-15 | Sequencing followed as planned; generated doc *tables inside the hand-authored docs* remain the one unstarted piece (tokens.md generation exists as the pattern). |

Open items carried forward: a `pop` reveal inside a `lay-out-group` is trapped by the group's containment (documented limitation) and its open-state tier-pause now resolves against the group container (unobserved, flagged). *(F-12 scrim-declaration scoping and the `-ink` alias retirement shipped later, in the v5 alias batch — see the TODO below.)*

**Closeout round (2026-07-27, third round)** — resolved the remaining calls and quick wins, after merging upstream v4 carousel work (`mrk(sbr)`/`mrk(lbl)` added to the manifest on merge):

- **Doc-table generation (R-15's last piece).** `tokens.build.js` now injects generated tables into the hand-authored docs between `<!-- tokens:… -->` markers (four generic block types: summary / args / matrix / aliases); 17 drift-prone inventory tables across `media.md`, `content.md`, `carousel.md`, `ui-card-tokens.md`, `card.md`, `readme.md`, `reveal/readme.md` are now build-owned, and the discrepancy list's 16 doc corrections were applied to the surrounding prose. Custom-property/typography-ladder tables stay hand-written (the manifest has no defaults/descriptions to generate from).
- **Design calls.** `bdr` on `<ui-reveal>` now paints on `> details` (the rounded surface); group-header base size tokens ride the responsive `scl()` ladder via `lay-out-group > cq-box` arms; the slide-wrapper contradiction resolved code-wins (`<lay-out>` is not a slide — docs corrected); the polyfill keeps its local `NOT_SLIDE` mirror by design, with drift now a `tokens.lint.js` failure (CSS ⊆ shared.js, polyfill == shared.js).
  > **Superseded (collage work).** The slide-wrapper call was resolved the wrong way round. The CSS `:not()` never excluded `LAY-OUT`, so a `<lay-out>` direct child *is* a slide — it snaps and takes a `::scroll-marker`. That is now a shipped feature (the CSS-only collage carousel), and the two lists differ on purpose: only `slidesOf()` excludes `LAY-OUT`, so the JS features no-op on such a carousel. See `ui/card/carousel.md` and `ui/card/media.md` § Collage. The `NOT_SLIDE` mirror + lint conclusion still stands.
...
- [x] **v5 alias batch — DONE.** F-12 (scrim declarations scoped to `:where(ui-media), :where([media*="scm"]), :where([variant*="ovr("])` — the exact readers; the blanket `[media]`/`[variant]` arms are gone), hue-alias retirement (`dark`/`light`/`subtle` **removed**; `slate` **promoted to a canonical ninth hue** — it always routed to its own `--ui-theme-slate-*` bundle, so removing it would have deleted a colour), the F-09 `-color` → `-ink` alias slots **removed** (7 author-facing custom properties), and the physical `tl…br` position aliases **removed** from `ovr()` — leaving `obp()` as the system's only physical vocabulary, which it keeps by design. The polyfill's `mrk(tl/tr/bl/br)` arms (never manifest-declared, so the canonical spellings had no polyfill corner placement) were renamed to the logical cells in the same pass.
- [x] **R-14 step 4 (`style()`-flag migration) — PARTIAL, and the remainder is a structural limit, not remaining work.** Migrated: `marquee(top|bot)` placement, `shp()`'s image clip-path, `tnt`'s paint + `hov(tint)`'s fade, and the **entire `hov()` family** (14 rules). Each is now one combinator-free `:where([media*="tok"])` flag setter — which matches the host arm and the self arm at once, because the flag *inherits* — plus one `@container style(--_flag: …)` block, with a single boundary reset at the top of `media.css` (`:where(ui-card, ui-reveal) :where(ui-card, ui-reveal) { --_…: initial }`, declared before every setter so an inner host carrying its own token re-wins on source order) preserving the media=-stops-at-the-card contract. Two mechanics were verified in Chromium before any edit: a `::before` resolves its style query against its **originating** element (which is what makes `tnt` migratable at all), and a real property landing on the attribute-bearing element is reachable from the host arm but **not** the self arm. That second result is the boundary of the whole technique — a container cannot restyle itself — so `asr()`'s `min-block-size`, `rds(*-sq)`'s `corner-shape`, `shp()`'s frame `background`, `tnt`'s `isolation`, `hov(tilt*)`'s `perspective` and essentially the **whole carousel bundle** (its real properties *are* the scroller box: `display`/`overflow`/`box-sizing`/`padding`/`inline-size`/`scrollbar-*`) keep their two-arm form permanently. The carousel was therefore assessed and deliberately **not** touched: only ~5 of its ~18 rules have a child subject, and splitting the file between two idioms would cost more than the duplication it removes. `flp()`/`obf()`/`obp()` — listed as candidates — turned out to write only custom properties (`realProperties: false` in the manifest), so they never had a dual arm. Parity: computed-style fingerprints at 400/1100px across 10 demo pages, a hover rig that hovers **both** the frame and its host for all 384 frames on 7 pages with transitions zeroed, and a `prefers-reduced-motion` pass — all byte-identical to the pre-change tree. Support posture (style queries: Chrome 111+/Safari 18+/Firefox 128+) documented in `media.md` § "v5 support posture" and `readme.md`.
