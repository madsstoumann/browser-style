...
Open items carried forward: a `pop` reveal inside a `lay-out-group` is trapped by the group's containment (documented limitation) and its open-state tier-pause now resolves against the group container (unobserved, flagged); F-12 scrim-declaration scoping and the `-ink` alias retirement wait for v5.
...
- **Layout quick wins.** F-35 flat-config rename (`srcsetConfig`), F-36 demo repair + stale `.tmp` plan removal, F-37 generated container CSS layered into `@layer layout.base` with dist rebuilt. New follow-ups surfaced: `layout/package.json` points at a nonexistent `index.js` (publish-breaking), and `build:maps` drops `xs: 240` (generator skips min-less breakpoints — checked-in map is stale vs. its generator, flagged `KNOWN DRIFT`).
