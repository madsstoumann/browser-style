...
## 38. `theme=` on a `<lay-out>` paints the plate but not the ink — DONE 2026-09-01

Both chains now read the full theme fallback —
`var(--layout-c, var(--_theme-c, var(--_theme-ink, inherit)))` in `layout/core/base.css`
and the group equivalent in `core/group.css` — identical to every other consumer (card,
reveal, chip). Measured after (bare themed `<lay-out><p>` probe): `black` **1.03 →
13.46**, `red` **2.21 → 6.84**, `gray` unchanged at 12.93. The `muted` trade was
accepted knowingly (muted ink is 50%-translucent by design); the one live themed layout
(`media.collage.html` `theme="muted gray"`) was verified unaffected — its computed ink
is solid `rgb(38,38,38)` and the card inside resolves its own `--color-text`.
`layout/core/base.md`'s wrong "theme ink wins" claim corrected in the same change;
layout dist rebuilt.
