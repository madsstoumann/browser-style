...
## 36. Google Rich Results reports 2 more valid items from JSON-LD than from microdata

**Where:** `demo/schema.html` (microdata) vs `demo/schema.jsonld.html` (JSON-LD)

Measured 2026-08-20 on the deployed pages:

| Tool | Microdata | JSON-LD |
|---|---|---|
| schema.org validator | **63 items** | **63 items** |
| Google Rich Results | 71 valid items | **73 valid items** |

**The schema.org validator agreeing exactly at 63 is the headline** — it is independent
confirmation that `jsonld.js` reproduces the microdata graph without loss or invention, which
is the property the whole extraction approach rests on.

**The +2 in Google's tool is unexplained and worth one look.** Both pages assert the same
graph, so a difference is about how Google *parses*, not what we state. The likely candidates,
in order: Google splitting a nested node into its own item in one syntax but not the other;
an item whose microdata scope is ambiguous because the property rides an element that also
carries text; or simply a different counting rule for the two formats. It is a small
discrepancy in our favour, so it is a curiosity rather than a defect — but if microdata is
losing two items to a scoping quirk, that quirk is worth knowing about, since microdata is
