...
- **Scroll-spy binds by number, on both sides.** `ui/scroll-spy` gives every target
  `view-timeline: --spy-n` and every nav item `animation-timeline: --spy-n` through ONE
  attribute, `data-spy="n"` on each — a timeline name is an ident and `sibling-index()` a
  number, so the binding is twelve one-line `[data-spy="n"]` arms and `timeline-scope` is one
  non-additive declaration on `:root:has(…)` (12 targets, one spy per page). The spec's
  `ident("--spy-" attr(data-spy))` would delete the arms; it is unshipped in Chrome 152 and
  Safari 26.5 (`CSS.supports` false, checked 2026-08-28) — re-test before "simplifying". Ranges use
  **`exit-crossing`, never `exit`**: for a subject taller than the inset scrollport `exit 0%`
  = `contain 100%` (bottom edge at the viewport bottom), so a tall section would light up
  late. `stagger=` and `data-spy` cannot share an element (both set `view-timeline`). The
  SSR pages emit both sides from one array (`build.shared.js` § `scrollSpy()`/`spyAttrs()`).
  Docs: ui/scroll-spy/readme.md § How it works.
