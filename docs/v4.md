...
| `ui/card/data/details.json`, a `details` field, or a schemaType's editor fields | `node ui/card/details.build.js` ×2 (2nd a no-op) → `node ui/card/details.lint.js` → 3 → 6 — regenerates `cms/editors/card/src/details.data.js`, the `card.schema.json` options/digest and the card.model.md tables; commit them together |
