...
4. **Content models** — `cms/baseline/`. **UCM** (Unified Content Model,
   `pages/UCM.md`) defines content *types*; **UCF** (Unified Content Format,
   `pages/UCF.md`) defines content *instances* — the `$ref` / `$asset` / `$richtext`
   markers every `ui/card/data/*.json` uses. The card's own model is
   `models/card.schema.json`, and `ui/card/docs/card.model.md` is the editor-facing
   view of it.
5. `layout/AGENTS.md` — the `<lay-out>` system: viewport breakpoints (xs 240 base /
...
6. Per-topic docs when relevant, all under `ui/card/docs/`: `card.md` (content model +
   renderer), `card.model.md` (the same model as a CMS/editor sees it — field types,
   per-type `details`, every dropdown vocabulary), `media.md`, `content.md`, `typography.md`, `carousel.md` +
...
7. `docs/performance.md` — the CPU/GPU split: which properties composite, the four
   ...
   `perf-pass` skill. The transfer-weight budget and the W3C Web Sustainability
   Guidelines status map sit beside it in `docs/sustainability.md`.
8. `docs/guide.md` — the narrative user guide (card → section → page) when you need
...
- **Icon markers: the correction is in the FONT, not the CSS.** A `<li data-icon="bed">`
  ...
  `<link>`, and without it rows fall back to a normal bullet. The demo pages honour that:
  since 2026-09-07 every one carries a SECOND `<link>`, `/dist/icon-font.<hash>.min.css`
  (entry `ui/card/demo/icon-font.entry.css`), instead of the `@import` the demo bundle used
  to hide it behind — the bundle rehashes far more often than the icon set changes. Its
  leading `@layer bs-core, bs-component;` keeps link order from inverting base's cascade.
  Adding an icon = edit
...
| `cms/baseline/models/card.schema.json` | 3 → 6 (`schema.compare.js` is the contract); `node ui/card/details.build.js && node ui/card/details.lint.js` if `details` or the type list moved |
| `ui/card/data/details.json`, a `details` field, a `ref` projection, or a schemaType's editor fields | `node ui/card/details.build.js` ×2 (2nd a no-op) → `node ui/card/details.lint.js` → 3 → 6 — regenerates `cms/editors/card/src/details.data.js`, `data/details.refs.data.js` (the renderer's ref-projection table), the `card.schema.json` options/digest and the card.model.md tables; commit them together. The lint's rules 11/12 hold every `{ "$ref": "card/<id>" }` detail row against the corpus and the committed refs mirror |
