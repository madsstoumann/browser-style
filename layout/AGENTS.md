...
### Page-gap: `data-layout-root` + `page-gap`
...
parent). `page-gap="N"` = `N × --layout-space-unit` (default `1`; the fallback
`1rem` is needed because `--layout-space-unit` is declared on `<lay-out>`, a
descendant, not an ancestor). Because it's `row-gap` on the direct parent,
