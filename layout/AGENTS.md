...
Scroll-driven animations via `animate-self` (container) and `animate` (children) attributes. CSS lives in `core/animations.css` (static, not generated by the builder). Full documentation in `core/animations.md`.
...
Creates a named `view-timeline: --layout-tl` on the container; children animate independently using that timeline with staggered offsets.
...
Controls animation speed via `pace` attribute (space-separated tokens).
