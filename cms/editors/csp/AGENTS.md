...
| Property | Value |
|----------|-------|
| Custom Element | `<editor-csp>` |
| Shadow DOM | Yes (open mode, adoptedStyleSheets) |
| Dependencies | `@browser.style/editor-shared` |
| i18n Support | English, Danish |

## Installation

```bash
npm install @anthropic/editor-csp
```
...
```javascript
const csp = document.querySelector('editor-csp');
await csp.ready;
...
The component uses the `createTranslator` utility from editor-shared for i18n support.
...
```html
<editor-csp i18n="./my-translations.json" lang="de"></editor-csp>
```
...
The component inherits styling from editor-shared and adds evaluation-specific styles:
...
editor-csp/
├── src/
│   ├── index.js          # Main EditorCsp component
...
├─────────────────────────────────────────────────────────────┤
│  Dependencies                                                │
│  ├── editor-shared (setState, createTranslator, styles) │
...
### State Management
Uses `setState` from editor-shared for reactive updates:

```javascript
import { setState } from '@browser.style/editor-shared';
...
### Translation System
```javascript
import { createTranslator } from '@browser.style/editor-shared';
...
<head>
  ...
  {
    "imports": {
      "@browser.style/editor-csp": "./src/index.js",
      "@browser.style/editor-shared": "../editor-shared/src/index.js"
    }
  }
  </script>
</head>
<body>
  ...
  <script type="module">
    import '@browser.style/editor-csp';
