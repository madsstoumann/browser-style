...
const _localReady = _localSheet.replace(`
	...
	label small {
		color: var(--editor-color-muted, #666);
	...
	th, td {
		border: 1px solid var(--editor-bdc, #ddd);
		padding: 0.35em 0.75em;
		text-align: start;
	}
	th {
		background: var(--editor-bg-muted, #f5f5f5);
		font-weight: 600;
	}
	...
	.error.active {
		...
		border: 1px solid var(--editor-status-danger);
	...
	.spinner.active::after {
		...
		width: 20px;
		height: 20px;
		border: 2px solid var(--editor-bdc);
		border-top-color: var(--editor-accent-dark);
		border-radius: 50%;
...
class EditorImageText extends HTMLElement {
...
customElements.define('editor-image-text', EditorImageText);
