...
## Compatibility with `<editor-tokens>`

This architecture is **fully compatible** with the existing `editor-tokens` component ecosystem:

| Component | Role | Compatibility |
|-----------|------|---------------|
| `<editor-tokens>` | Visual editor for token files | Loads/saves `design.tokens.json` |
...
CMS Page
    ...
    ▼
┌─────────────────────────────────────────────────────────────┐
│  <editor-tokens src="/api/tokens/site-a">               │
│                                                             │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐         │
...
1. **Loading**: `<editor-tokens src="...">` fetches JSON via `src` attribute
...
- `$extensions.ui.title` provides display names in `<editor-tokens>`
...
## CMS Integration with `<editor-tokens>`
...
<script type="module">
  import '/ui/editor-tokens/src/index.js';
...
| **Visual editor** | `<editor-tokens>` component |
