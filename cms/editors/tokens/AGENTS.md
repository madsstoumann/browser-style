# Editor Tokens - Internal Architecture

## Overview

Editor Tokens is a **web component for visualizing and editing W3C design tokens** with live preview and CSS generation. It serves as the main container that orchestrates the design token ecosystem, providing tree-based navigation and CRUD operations for token management.

**Component Type:** Web Component (Custom Element)

**Tag Name:** `editor-tokens`
...
**Lines 196-647:** `EditorTokens extends HTMLElement`

**Registration:** Line 647: `customElements.define('editor-tokens', EditorTokens);`
...
styles.replaceSync(`
  :host {
    --editor-tokens-gap: clamp(0.375rem, 0.125rem + 1vw, 0.75rem);
    display: block;
...
```css
details {
  padding-inline-start: calc(attr(data-level type(<number>)) * var(--editor-tokens-gap));
}
```
...
```html
<editor-tokens src="design.tokens.json"></editor-tokens>

<script type="module">
  import './editor-tokens/src/index.js';

  const tokenEditor = document.querySelector('editor-tokens');
