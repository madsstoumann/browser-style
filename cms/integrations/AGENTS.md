...
Load Contentful SDK from CDN (unpkg.com)
  ↓
Define import map for @browser.style/editor-shared
  ↓
...
contentfulExtension.init(async (api) => { ... })
  ↓
  customElements.whenDefined('editor-xxx')
  ↓
...
### editor-manifest/index.html (72 lines)
...
### editor-robots/index.html (87 lines)
...
### editor-security/index.html (87 lines)
...
### editor-image-text-alttext (Contentful + Umbraco)

AI-powered accessible image editor. Uses `<editor-image-text preset="alttext">` to generate alt text and long descriptions from images via Claude Vision API.

**Component:** `<editor-image-text>`
...
├── media        (Asset link)
├── alt          (Short text, editor: editor-image-text-alttext app)
...
<script type="importmap">
  {
    "imports": {
      "@browser.style/editor-shared": "https://browser.style/ui/editor-shared/index.js"
    }
  }
</script>
```
...
```javascript
await customElements.whenDefined('editor-xxx');
const editor = document.createElement('editor-xxx');
...
| editor-robots | `<editor-robots>` | `robtxt-change` | `{ robotsTxt }` | `setValue(robotsTxt)` |
| editor-security | `<editor-security>` | `sec-change` | `{ securityTxt }` | `setValue(securityTxt)` |
...
Apps served from `/cms/integrations/contentful/*` require special headers:
...
1. Create directory: `/cms/integrations/contentful/[component-name]/`
