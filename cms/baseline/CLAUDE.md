...
- Built with Docusaurus, served at `https://browser.style/cms/baseline/`
- `baseUrl: '/cms/baseline/'` in `docusaurus.config.js`
...
- `card` — the current polymorphic card model: a shared envelope plus one `details` object per
  `schemaType` (54 types). Presentation lives outside it, in a referenced `card-preset`
